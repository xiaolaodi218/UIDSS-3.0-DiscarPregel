...
spark-submit --class cn.ctyun.UIDSS.UIDSS  --master yarn     --deploy-mode cluster  --files /home/dal_pro/dal_pro.keytab  --driver-memory ${driver_memory}  --executor-memory ${executor_memory}  --num-executors ${num_executors}  --executor-cores  ${executor_cores}  --queue ${yarn_queue}  UIDSS-3.0-jar-with-dependencies.jar Y_DeleteOldData ${obsolete_date}

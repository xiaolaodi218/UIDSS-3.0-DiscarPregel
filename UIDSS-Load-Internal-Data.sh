...
do
  if [ "${pre_month}" = "0" ] ; then
    ...
    spark-submit --class cn.ctyun.UIDSS.UIDSS  --master yarn     --deploy-mode cluster --files /home/dal_pro/dal_pro.keytab --driver-memory ${driver_memory_UID_INFO_MBL}  --num-executors ${num_executors_UID_INFO_MBL}    --executor-memory  ${executor_memory_UID_INFO_MBL}  --executor-cores  ${executor_cores_UID_INFO_MBL}  --queue ${yarn_queue}  UIDSS-3.0-jar-with-dependencies.jar  Y_LoadRawData UID_INFO_MBL   ${hdfs_base_dir}/${mbl_dir}/prov_id=${array_province[i]}/month_id=${cur_month}/ 0
    wait
    #Internal fix-line user data
    spark-submit --class cn.ctyun.UIDSS.UIDSS  --master yarn     --deploy-mode cluster --files /home/dal_pro/dal_pro.keytab --driver-memory ${driver_memory_UID_INFO_TEL}  --num-executors ${num_executors_UID_INFO_TEL}    --executor-memory  ${executor_memory_UID_INFO_TEL}  --executor-cores  ${executor_cores_UID_INFO_TEL}  --queue ${yarn_queue}  UIDSS-3.0-jar-with-dependencies.jar  Y_LoadRawData UID_INFO_TEL   ${hdfs_base_dir}/${tel_dir}/prov_id=${array_province[i]}/month_id=${cur_month}/ 0
    wait
    #Internal wide-band user data
    spark-submit --class cn.ctyun.UIDSS.UIDSS  --master yarn     --deploy-mode cluster --files /home/dal_pro/dal_pro.keytab --driver-memory ${driver_memory_UID_INFO_WB}  --num-executors ${num_executors_UID_INFO_WB}    --executor-memory  ${executor_memory_UID_INFO_WB}     --executor-cores  ${executor_cores_UID_INFO_WB}    --queue ${yarn_queue}  UIDSS-3.0-jar-with-dependencies.jar  Y_LoadRawData UID_INFO_WB   ${hdfs_base_dir}/${wb_dir}/prov_id=${array_province[i]}/month_id=${cur_month}/ 0
  else
